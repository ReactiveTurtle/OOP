...
int End(string result, bool wasError);
...
int main(int argc, char* argv[])
{
	...
	return End(result, convertModel.wasError);
}
...
int StringToInt(const string& str, int radix, bool& wasError)
{
	...
	for (int i = 0; i < strNumber.length() && !wasError; i++)
	{
		...
		if (isNegative) {
			result -= chNumber * (int)pow(radix, strNumber.length() - i - 1);
		}
		else {
			result += chNumber * (int)pow(radix, strNumber.length() - i - 1);
		}
